...
*   **文档分析**：可上传 PDF/TXT/MD 文档。PDF 会优先尝试调用本机 `docling` 进行高级 Markdown/表格解析；如果未安装、解析超时或失败，会自动回退到 `pdf-parse` 兼容解析，并在上传区显示明确提示。
...
首次使用可能会下载本地解析模型，耗时取决于网络和机器性能。扫描件 OCR/VLM 高精度模式暂未默认启用。
