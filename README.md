...
*   **文档分析**：可单选或多选上传 PDF/TXT/MD 文档。PDF 默认使用 `pdf-parse` 快速兼容解析，适合医生日常上传试用；也可在输入框下方开启“高级PDF解析”，尝试调用本机 `docling` 生成 Markdown/表格文本，失败或超时会自动回退到兼容解析，并在上传区显示明确提示。
...
Docling 是可选增强，不安装也能上传 PDF。若要启用输入区的“高级PDF解析”，请确保系统命令行可执行 `docling`：
...
首次使用可能会下载本地解析模型，耗时取决于网络和机器性能。扫描件 OCR/VLM 高精度模式暂未默认启用。高级解析当前默认等待时间为 30 秒，超时会自动回退到兼容解析；如需做离线验收，可设置 `YUNWU_DOCLING_TIMEOUT_MS=300000` 临时放宽到 5 分钟。
...
暂未纳入本试用版：
*   高级 PDF 版面/表格解析仍作为可选尝试，真实医学论文评测显示当前机器上 Docling 交互式体验不稳定，下一版再继续优化。
